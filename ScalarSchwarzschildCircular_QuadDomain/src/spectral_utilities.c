...
void Chebyshev_Coefficients_Radau_RHS(double *psi, double *c, int N)
{
	...
	for(m=0; m<=N; m++){
		...
		if(m==0) c0 = c[m];
		if(fabs(c[m]/c0)<ChebyCoeff_cut) c[m] = 0.;
	}
...
{ 
	int i1,  i2, k1max = 2*floor(0.5*N1), k2max = 2*floor(0.5*N2);
	double sum1 = 0., sum2 = 0., sum12 = 0.;
	
	for(i1=k1max; i1>=2; i1 -= 2) sum1 += c[i1][0]/(-1. + i1*i1);

	for(i2=k2max; i2>=2; i2 -= 2) sum2 += c[0][i2]/(-1. + i2*i2);

		for(i1=k1max; i1>=2; i1 -= 2)
			for(i2=k2max; i2>=2; i2 -= 2)
				sum12 += c[i1][i2]/((-1. + i1*i1)*(-1. + i2*i2));
	
	return c[0][0] - 2.*sum1 - 2*sum2 + 4*sum12;
}