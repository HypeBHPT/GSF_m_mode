...
void output_Toy2ndSource(parameters par, double *X, int N, int N_min, int N_max, int delta_N, int m, int m_min, int m_max){
  ...
  int iDom;
  FILE *fp_m_RR, *fp_m_SR;
  char fn_n_RR[1000], fn_n_SR[1000];
  derivs_2D Sol;
  
   
  double L2_Toy2ndSource_RR, L2_Toy2ndSource_SR;
  allocate_derivs_2D(&Sol, par.ntotal);
  get_Solution(par, X, Sol);
  
  for(iDom=0; iDom<nDom; iDom++){	
    get_ToyScndOrdSource_L2Norm(par, iDom, Sol, &L2_Toy2ndSource_RR, &L2_Toy2ndSource_SR);					
    sprintf(fn_n_RR, "data/%s/../../L2_Toy2ndSource_RR_mDep_%d_%d_dom%d.dat", par.SimName, m_min, m_max, iDom);
    sprintf(fn_n_SR, "data/%s/../../L2_Toy2ndSource_SR_mDep_%d_%d_dom%d.dat", par.SimName, m_min, m_max, iDom);
    
    if(m==m_min && N==N_max){			
      fp_m_RR = fopen(fn_n_RR, "w");							
      fprintf(fp_m_RR, "#1:m\t ");

      fp_m_SR = fopen(fn_n_SR, "w");							
      fprintf(fp_m_SR, "#1:m\t ");
      
      int iN, count=0;
      for(iN=N_max; iN>=N_min; iN-=delta_N){
        fprintf(fp_m_RR, "#(%d):L2_Toy2ndSource_RR (N=%d)\t ", count+2, iN);
        fprintf(fp_m_SR, "#(%d):L2_Toy2ndSource_SR (N=%d)\t ", count+2, iN);
        count++;
      }
      fprintf(fp_m_RR, "\n");
      fprintf(fp_m_SR, "\n");
      ...
      fp_m_RR = fopen(fn_n_RR, "a");
      fp_m_SR = fopen(fn_n_SR, "a");
    }
    if(N==N_max){
      fprintf(fp_m_RR, "%d\t ", m);
      fprintf(fp_m_SR, "%d\t ", m);
    }								
    fprintf(fp_m_RR, "%3.15e\t ", L2_Toy2ndSource_RR);
    fprintf(fp_m_SR, "%3.15e\t ", L2_Toy2ndSource_SR);
    if(N==N_min){
      fprintf(fp_m_RR, "\n");
      fprintf(fp_m_SR, "\n");
    }
