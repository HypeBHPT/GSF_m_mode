...
func_sigma_derivs_2D Test_Func(parameters par, double sigma, double y){
  ...
  f.d0 = sig2*y2;//sig2*y2/den; //

  f.dsigma = 2*sig*y2; //2*y4*sig/den_2; // 
  f.d2sigma =2*y2; // 2*y4*(y2 - 3.*sig2)/den_3; // 

  f.dy =2*y*sig2; //2*y*sig4/den_2; // 
  f.d2y = 2*sig2; //2*sig4*(sig2 - 3.*y2)/den_3;//

  f.d2sigmay = 4*y*sig; //8*y3*sig3/den_3;//
...
double complex Test_Effective_Source(parameters par, int iDom, int i1, int i2){
  ...
  double complex HypSource, a2, a1, a0, g2, g1;
  double chi_1, chi_2;
      ...
      for(j1=0; j1<=j1_max; j1++){
        ...
        for(j2=0; j2<=j2_max; j2++){
          ...
          double out = Clenshaw_Chebyshev_2D(c, N1, N2, chi_1, chi_2), 
                 error = iF == 0? fabs(f.d0 - out):fabs(out);
          fprintf(fp, "%3.15e\t%3.15e\t%3.15e\n", sigma.d0, y.d0, error);
        }
        fprintf(fp, "\n");
      }
...
void Check_Derivative(parameters par, char *fn, int j1, int j2, derivs_2D W, int FLAG){
  ...
  func_sigma_derivs_2D f = Test_Func(par, sigma.d0, y.d0);
	...
	double err_fsig = fabs(f.dsigma - creal(phi.dsigma))+1.e-20;
	double err_fsigsig = fabs(f.d2sigma - creal(phi.d2sigma))+1.e-20;
	double err_fy = fabs(f.dy - creal(phi.dy))+1.e-20;
	double err_fyy = fabs(f.d2y - creal(phi.d2y))+1.e-20;
	double err_fsigy = fabs(f.d2sigmay - creal(phi.d2sigmay))+1.e-20;
