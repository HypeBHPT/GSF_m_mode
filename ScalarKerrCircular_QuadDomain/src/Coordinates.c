  ...
  switch(iDom){
	case 0: //Dom Scri
		if(iDom!=par.Dom_scri){
			fprintf(stderr, "Error in get_sigma: idom = %d dom_scri=%d\n", iDom, par.Dom_scri);
			exit(-1);
		}
  		//Map sigma(x1,x2)
		(*sigma).d0 = 0.5*(1. + x1)*par.sigma_minus;
		//derivative x1
		(*sigma).d1 = 0.5*par.sigma_minus; 
		//derivative x1 x1
		(*sigma).d11 = 0.;
		//derivative x1 x2 
		(*sigma).d12 = 0.; 
  	...
  	case 1: //Near-Particle Vacuum 
		if(iDom!=par.Dom_bulk){
			fprintf(stderr, "Error in get_sigma: idom = %d dom_bulk=%d\n", iDom, par.Dom_bulk);
			exit(-1);
		}
		//Map sigma(x1,x2)
  	...
  	case 2:	//Near-Particle Excision
		if(iDom!=par.Dom_ptcl){
			fprintf(stderr, "Error in get_sigma: idom = %d dom_particle=%d\n", iDom, par.Dom_ptcl);
			exit(-1);
		} 
		rho = (1.+x2)/2. + par.rho_min*(1-x2)/2;
        rho_2=sqr(rho);       
		drho_dx2 = 1./2 - par.rho_min/2;  		
		d2rho_dx2x2 = 0.;
	...
	case 3: //Dom Horizon
		if(iDom!=par.Dom_hrzn){
			fprintf(stderr, "Error in get_sigma: idom = %d dom_horizon=%d\n", iDom, par.Dom_hrzn);
			exit(-1);
		}
		//Map sigma(x1,x2)
		(*sigma).d0 = 0.5*(1. - x1)*par.sigma_plus + 0.5*(1. + x1);
		//derivative x1
		(*sigma).d1 = -0.5*par.sigma_plus + 0.5;
		//derivative x1 x1
		(*sigma).d11 = 0.;
		//derivative x1 x2 
		(*sigma).d12 = 0.; 
...
void get_y(parameters par, int iDom, double chi_1, double chi_2, func_derivs_2D *y){
  ...
  double eta = par.eta, eta_2 = sqr(eta), x1_2 = sqr(x1),
   ...
   switch(iDom){
	case 0: //Dom Scri
		if(iDom!=par.Dom_scri){
			fprintf(stderr, "Error in get_y: idom = %d dom_scri=%d\n", iDom, par.Dom_scri);
			exit(-1);
		}
   		//Map y(x1,x2)
		(*y).d0 = 0.5*(1. + x2);
	...
	case 1://Near-Particle Vacuum
		if(iDom!=par.Dom_bulk){
			fprintf(stderr, "Error in get_y: idom = %d dom_bulk=%d\n", iDom, par.Dom_bulk);
			exit(-1);
		}
		//Map y(x1,x2)
	...
	case 2: //Near-Particle Excision
		if(iDom!=par.Dom_ptcl){
			fprintf(stderr, "Error in get_y: idom = %d dom_particle=%d\n", iDom, par.Dom_ptcl);
			exit(-1);
		}
		rho = (1.+x2)/2.+ par.rho_min*(1-x2)/2;
        drho_dx2 = 1./2 - par.rho_min/2;
	...
	case 3: //Dom Horizon
		if(iDom!=par.Dom_hrzn){
			fprintf(stderr, "Error in get_y: idom = %d dom_horizon=%d\n", iDom, par.Dom_hrzn);
			exit(-1);
		}
		//Map y(x1,x2)
		(*y).d0 = 0.5*(1. + x2);
...
void get_chi1_chi2_From_sigma_y(parameters par, int iDom, func_derivs_2D sigma, func_derivs_2D y, double *chi_1, double *chi_2){
  ...
  double sigma0 = par.sigma0, rho, dsigma = sigma.d0-sigma0,
    ...
    switch(iDom){
		case 0: //Dom Scri
			if(iDom!=par.Dom_scri){
				fprintf(stderr, "Error in get_chi1_chi2_From_sigma_y: idom = %d dom_scri=%d\n", iDom, par.Dom_scri);
				exit(-1);
			}
    		x1 = 2*sigma.d0/par.sigma_minus - 1.;
    		x2 = 2*y.d0 - 1.;
    	break;
	  
	  case 1:
	 	 if(iDom!=par.Dom_bulk){
			fprintf(stderr, "Error in get_chi1_chi2_From_sigma_y: idom = %d dom_bulk=%d\n", iDom, par.Dom_bulk);
			exit(-1);
		 }
  	...
  	case 2:
		if(iDom!=par.Dom_ptcl){
			fprintf(stderr, "Error in get_chi1_chi2_From_sigma_y: idom = %d dom_particle=%d\n", iDom, par.Dom_ptcl);
			exit(-1);
		}
  	...
  	x2 =( 2*rho - (1.+par.rho_min))/(1-par.rho_min);
	...
	case 3: //Dom Horizon
		if(iDom!=par.Dom_hrzn){
			fprintf(stderr, "Error in get_chi1_chi2_From_sigma_y: idom = %d dom_horizon=%d\n", iDom, par.Dom_hrzn);
			exit(-1);
	}
  		x1 = (2*sigma.d0 - (1.+par.sigma_plus))/(1. - par.sigma_plus);
    	x2 = 2*y.d0 - 1; 
  	break;
